...
> 调研日期：2026-09-17
> 最近修订：2026-09-17（第三次）—— 🎯 **§六 新增 R4「小额生态位的适配」**，并指出 R4 与 R2/R3 性质不同（经济结构差异 vs 能力差异）。分层依据见 [commercialization.md](./commercialization.md) §零
> 第二次修订：⚠️ **分类更正**——本文档初版把 OAP 与 Vaara 一并归入"学术界"，**这是错的**。两者都是**在售产品**（APort 有 $499/$4,990 月费，Vaara 有付费 pilot 与商业许可）。新增 §零 的分类更正、厂商定价与牵引数据，并作废 §六 的 R1
...
| **R4** 🎯 | **小额生态位的适配**（2026-09-17 第六次修订新增） | OAP 与 Vaara **都是大额/高后果框架的产物**——Vaara 卖 EU AI Act 高风险系统合规，OAP 按席位托管收费。**美分级支付的粒度不在两者的经济模型里**。详见 [commercialization.md](./commercialization.md) §零 | ✅ **最强的一条**——不是"他们没做"，是"做了不划算" |

**已经全部失去的**：收据签名、Merkle 完整性、离线验证、执行前拦截、拒绝留痕、拒付证明、**证据主权**。**其中"非省略证明"原本被视为唯一可变现的东西，现在归零。**

> 📌 **留给 Ledgeroot 的准确表述是**：不是"三根支柱"，而是**"小额 402 支付生态位里的用户签名授权 + 零出境"**。能力层没有独占，**生态位层有**。
>
> ⚠️ **但 R4 与 R2/R3 的性质完全不同**：R2/R3 是"能力差异"，可以被对手用半年抹平；**R4 是"经济结构差异"，对手转身进来要放弃自己的商业模式**。所以真正的防守重心应该是 R4，而不是 R2/R3。详见 [vaara-competitive-analysis.md](./vaara-competitive-analysis.md) §六 §七。
