...
| 类 | 性质 | 条数 | 处理方法 |
|---|---|---|---|
| **A** | **与规模无关的真 bug** | 2 | ✅ **A1 已修（2026-09-17）**；A2 待修 |
...
| **0** | **A2（`seq` 分配）** ⚠️ **下一个** | 数据完整性；会伪装成 `tampered` 误报，侵蚀"三态验证"的可信度 |
| **1** | D1b（链改配置化） | 成本低（类型已就绪）；让**测试网 → 主网**变成改配置而不是改代码 |
| **2** | C1 的设计决定 | 它决定控制面 schema。**先决定再写代码，否则返工** |
| 3 | B1（索引）+ B3（批量/保留） | 没有它们，百万笔/月跑不起来 |
...
>
> ✅ **A1 已在黑客松前完成**——它原是唯一会在现场现形的 bug（demo 连续快跑正好放大那个窗口，崩一次就是"钱扣了没收据"）。
