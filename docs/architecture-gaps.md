...
| 类 | 性质 | 条数 | 处理方法 |
|---|---|---|---|
| **A** | **与规模无关的真 bug** | 2 | ✅ **两条均已处理（2026-09-17）**：A1 是真 bug（改了）；**A2 实测不是 bug，是性能悬崖**（也改了） |
...
> ⚠️ **这两条无论生态位怎么定都该修。** 它们不是性能问题，是正确性问题。**A1 已于 2026-09-17 修复。**

### A1. 结算先于收据写入 —— 崩溃窗口会导致重复付款

> ✅ **已修复（2026-09-17）。** 见 §1.1 的实现与验证。
>
> ⚠️ **本节原先提出的修法是错的**，保留原文以便追溯（见下方"修法方向"）。
...
>
> ❌ **原"修法方向"（已被推翻）**："先写一条 `pending` 收据，结算成功后再更新为 `paid` + `txHash`。"
>
> **这做不到。** `buildReceipt` 把 `id` 定义为内容的规范哈希（`receipt/builder.ts`），所以**收据的身份就是它的内容**。把 `status` 从 `pending` 改成 `paid` 会改变内容 → 改变哈希 → **下一张收据的 `prevHash` 就指向一个不存在的哈希**。链是 append-only 的，原地更新在结构上不可能。
>
> 真正可行的是**把预写记录放在链外**，见 §1.1。

#### 1.1 实际实现（2026-09-17）

**设计：一个链外的"支付意图"表，在动钱之前占位。**

`src/store/db.ts` 新增 `payment_intents(request_id PRIMARY KEY, intent_json, created_at)` 与三个方法：`beginPaymentIntent` / `clearPaymentIntent` / `getPaymentIntent`。

`src/tools/pay.ts` 的已付路径变成：

| 步 | 动作 |
|---|---|
| 1 | `getReceiptByRequestId` 命中 → 直接回放（不变） |
| 2 | 策略放行后、**动钱之前**：`beginPaymentIntent(requestId)` 占位 |
| 3 | **占位失败**（已被占）→ **拒付**，记一条 `denied` 收据，理由写明"该请求有一次结果未被记录的支付尝试" |
| 4 | `payments.pay()` |
| 5 | 成功 → `record()` 写收据 → `clearPaymentIntent()` 释放 |

**为什么"占位失败"必须拒付而不是重试**：那一行的含义是"**我们不知道那笔支付有没有结算**"。拒付是可恢复的，重复付款不是。

**幂等键的语义因此变了**：`requestId` 在**尝试时**被消耗，而不是在成功时。这是非事务型轨道上幂等键唯一正确的语义。

**关于 `pay()` 抛异常（本次刻意保守）**：占位**不释放**。因为"调用失败"不等于"支付没发生"——`/settle` 的响应可能只是丢了。代价是：如果轨道确实明确拒绝了（例如 `/verify` 返回 `isValid:false`，钱肯定没动），这个 `requestId` 也会被占住，需要人工清理。**这是刻意选的方向：卡住一个 key 比多付一笔便宜。**

**验证（真实进程死亡，不是 mock）**：

```
1) 进程在结算后、写库前死亡      → exit 9，收据未写
2) 新进程用同一 requestId 重试    → settled=false（轨道未被再次调用）
                                  → status=denied，理由为"outcome was never recorded"
3) 磁盘状态                       → payment_intents 保留了该 requestId
                                  → 账本里是一条 denied 收据
```

**关键证据是第 2 步的 `settled=false`**——修复前 `getReceiptByRequestId` 是唯一的闸门，在这个状态下查不到任何东西，于是会再付一次。

**测试**：`test/pay.test.ts` 新增 4 个用例（占位存在 → 拒付且不调用轨道；成功后释放且重试回放；抛异常后占位保留且重试拒付；无 requestId 时不占位）。全库 **85 passed**。

**已知的后续项**：
- 没有"清理卡住的意图"的接口，目前要人工改库或换 `requestId`。若要自动化，需要让 `PaymentProvider` 区分"确定没结算"与"结果未知"——那要动 §七 提到的下一个接缝。
- 无 `requestId` 的调用**没有这层保护**（没有可对应的键）。幂等是必须主动要求的。

### A2. `seq` 用 `SELECT MAX(seq)+1` 分配 —— ⚠️ 原判断错误，真实问题是性能

> ✅ **已修复（2026-09-17）。** 见 §1.1。
>
> ⚠️ **原标题写的是"跨进程会撞号"。实测证明不会**——原因与实测数据见下。**正确性问题不存在，性能问题是真的，而且更严重。**
...
> ⚠️ **2026-09-17 实测更正：原判断错了。**
>
> 本节初版写的是"两个进程并发时会读到同一个 `MAX(seq)` → 产生重复 `seq`"。**实测不成立。**
>
> 那条 `INSERT` 是**单条 SQL 语句**，SQLite 在整条语句期间持有写锁，子查询与插入是原子的——**JS 层根本没有"读了再写"的窗口**。用两个连接交错追加 400 行，`seq` 是干净的 1..400，**零重复**。
>
> **所以 A2 不是正确性 bug。** 但实测暴露了一个更值钱的问题，见下。

#### 真实问题：每次插入一次全表扫描（实测）

`seq INTEGER NOT NULL` 上**没有索引**（`PRAGMA index_list` 只显示 `id` 的 PK 自动索引），所以 `SELECT MAX(seq)` 必须**扫全表**——而这张表每一行都存着完整的 `receipt_json`。
...
**10 倍数据 → 66 倍单次成本**，是悬崖不是斜坡。50,000 行时每笔支付光插入就要 **5.6 ms**，而这是一条每个 agent 调用都要走的路径。

> **修法（已实施）**：把 `seq` 变成 **rowid 别名**（`seq INTEGER PRIMARY KEY AUTOINCREMENT`），`id` 降为 `NOT NULL UNIQUE`。SQLite 自己分配 `seq`，**不需要扫描**，且唯一、单调、不复用（`AUTOINCREMENT` 保证删除后也不复用——这在以后加保留策略时会很重要）。
>
> 修后成本**基本平坦**（41 → 53 µs，10 倍数据），剩下的轻微上升是 `id` 唯一索引的 B-tree 深度，属正常。

#### 1.1 实现（2026-09-17）

`receipts` 的主键改型，SQLite 无法原地做，所以走**表重建**（建新表 → 拷贝 → 删旧 → 改名，全在一个事务里）。

**`seq` 的值不拷贝**，而是在重建时按追加顺序重新编号（`ORDER BY seq, rowid`）。理由：链靠 `prevHash` 连接，`seq` 只负责排序；重编号顺带修复了任何早于该列的行或带重复值的行。

旧的 `backfillReceiptSequence()`（逐行 `UPDATE` 回填）随之删除——它现在没有调用者。

**迁移路径覆盖**：
- `seq` 不存在（最老的形态）→ 重建，顺序取自 `rowid`
- `seq` 是普通列（中间形态，**真实库最可能在的状态**）→ 重建，顺序取自 `seq, rowid`
- 已是 rowid 别名 → 跳过

**验证**：`test/store.test.ts` 新增 3 个用例（中间形态的重建且保序、`seq` 必须是 rowid 别名这一结构断言、`seq` 从 1 连续递增）。全库 **88 passed**。基准数据见上表。
...
| 序 | 项 | 理由 |
|---|---|---|
| ~~0~~ | ~~A1（收据先于结算）~~ | ✅ **已完成（2026-09-17）**，见 §1.1 |
| ~~0~~ | ~~A2（`seq` 分配）~~ | ✅ **已完成（2026-09-17）**。**原判断（撞号）是错的**，真实问题是每次插入全表扫描——50k 行时 5.6 ms/笔，修后 53 µs |
| **1** | **D1b（链改配置化）** ⚠️ **下一个** | 成本低（类型已就绪）；让**测试网 → 主网**变成改配置而不是改代码 |
| **2** | C1 的设计决定 | 它决定控制面 schema。**先决定再写代码，否则返工** |
| 3 | B1（索引）+ B3（批量/保留） | ⚠️ **B1 的紧迫性上升**：A2 修掉的是写入侧的那一次扫描，但**每笔支付仍有 4 次未索引全表扫描**（读侧），性质相同 |
| 4 | D1c（主网 / 或补 RFC 3161） | 测试网锚定不产生证据价值。**黑客松后再做** |
| 5 | B2（增量验证）+ D2（超时） | ⭐ B2 与 N6 是同一条：**验证能力就是聚合能力的上限** |
| 6 | B4（多租户） | 舰队形态的前提，但可以晚于能跑通单 agent |

> 📌 **黑客松改变了上面这个排序**：现在该优化的是"**评委能不能看懂这是一个完整的 loop**"，不是装机量，也不是百万笔性能。所以 B 类整组（规模问题）都可以排在黑客松之后。
>
> ✅ **A1 与 A2 已在黑客松前完成。** A1 原是唯一会在现场现形的 bug；A2 修掉的那 5.6 ms/笔 在 demo 里也会变成肉眼可见的卡顿。
