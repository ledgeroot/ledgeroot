...
> **x402 的真实交易量在 Base 上。** 这不是"以后再说"——它意味着**今天跑不通真实场景**。`roadmap.md` §2.4 N3 已记录此差距，这里补充的是：它是**硬编码**，不是"缺个配置项"，改动会触及 `bootstrap.ts`、`chains.ts`、`onchain.ts` 与整个 facilitator 层。
>
> 📌 **可复用的部分**：`FacilitatorNetworkConfig` 已经把 `chainId` / `network` / `scheme` / `usdcAddress` / domain 抽出来了——**类型是对的，只是实例只有一个**。往前一步即可多链。
...
| 序 | 项 | 理由 |
|---|---|---|
| **1** | **A1（收据先于结算）** | ⚠️ 丢钱 + 丢证据，且违反核心承诺。**不该等生态位决定** |
| **2** | **A2（`seq` 分配）** | ⚠️ 数据完整性；会伪装成 `tampered` 误报，侵蚀"三态验证"的可信度 |
| **3** | **C1 的设计决定** | 它决定控制面 schema。**先决定再写代码，否则返工** |
| 4 | B1（索引）+ B3（批量/保留） | 没有它们，百万笔/月跑不起来 |
| 5 | D1（上 Base 主网） | 没有它，跑不了真实场景 |
