...
- ⚠️ **但它是通用 tool call 门控，不是支付层**；x402 在它那里只是下游 profile。**支付语义与"用户自签凭据"是 Ledgeroot 仅剩的两格。**

**关系定性**：不是"载波"（它不欢迎你插进它的格式），也不是"互补的另一半"（它不需要你的证据层）。**它是正面对手，而且是目前唯一在完整意义上正面对手的那个。**
...
### 6.2 但"支付 + 用户自签"这一格仍然真实

Vaara 可以加 x402 profile，但它不会把产品重心从"通用 tool call 门控"移到"agent 支付"——**它的表面稳定性承诺明确写了"No new primitives are planned"**。同理，它的 credential broker 模式是它的架构选择，改成"用户 EIP-712 自签"意味着重做授权模型。

**所以防守位是两条，不是三条**：**支付语义** + **用户自签凭据**。
