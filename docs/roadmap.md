...
| N10 ❗ | **单文件断网验证器** | **Vaara Resin**（单 HTML、断网） | ⚠️ 证据包内已达标（零依赖单文件 `verify.mjs`，只 import `node:crypto`，断网可跑）；尚未做成独立 npm 包或单 HTML | "零出境"最彻底的证明形式 |
...
→ **动作：补 N10（单文件断网验证器）。** 证据包内已经达到（零依赖、只 import `node:crypto`、断网可跑）；剩下的是分发形态（独立 npm 包 / 单 HTML）。在做成可分发形态之前，"本地优先"仍偏**平价而非优势**。
