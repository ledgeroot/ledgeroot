...
- **金额轴**（小额 ↔ 大额）——上面的费率论证只管这条：$0.005 在卡费率上不可行，所以我们不做大额。
- **受理轴**（消费端商户受理 ↔ 机器对机器）——**消费端 agent 支付跑在 Shop Pay / Stripe Link 上、用一次性令牌结算，稳定币插不进去；那里卡住的是商户接受度，不是费率。** 稳定币的位置在卡网络**不愿碰**的那头：**机器对机器、跨境长尾、API 级微支付**。
- **两条叙事不混**：不拿卡费率去解释消费端（那里的门槛是受理网络），也不宣称"消费端 agent 支付需要稳定币"——它不需要。
